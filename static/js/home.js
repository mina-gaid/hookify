...
const loadGenres = (genres) => {
  ...
  request.onload = () => {
    if (request.status == 200) {
      displayAlert(`<i class="fas fa-headphones-alt fa-lg mr-3"></i> Here's some ${toTitleCase(selected.replace("-", " "))} tracks recommended for you...`)
    ...
    if (request.status == 200) {
      ...
      if (parsed.hasOwnProperty('error')) {
        ...
        if (typeof newrelic == 'object') {
          newrelic.addPageAction('search-performed', {
  ...
  request.onload = () => {
    if (request.status == 200) {
      ...
      if (parsed.hasOwnProperty('error')) {
        ...
        if (typeof newrelic == 'object') {
          newrelic.addPageAction('user-clicked-find-similar', {
            'result': 'success'
          });
        }
        displayRecommendations(parsed)
...
const playSong = e => {
  ...
  request.onload = () => {
    if (request.status == 200) {
      ...
      displayAlert('<i class="fas fa-exclamation-triangle mr-3"></i> Could not load video.', "alert-danger")
    }
  };
  request.onerror = () => {
    parsed = JSON.parse(request.responseText)
